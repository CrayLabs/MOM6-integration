...
type, public :: smartredis_meke_CS_type; private
  ...
  real :: eke_max ! The maximum value of log(eke) considered physically reasonable
  ...
  integer :: nx_ny_t !< Number of points on the T-grid in the
...
subroutine smartredis_meke_init(diag, G, US, Time, param_file, smartredis_CS, CS)
  ...
  integer :: sr_return_code, batch_size
  character(len=40) :: mdl = "MOM_SMARTREDIS_MEKE"

  ! Store pointers in control structure
  CS%diag => diag
  CS%client => smartredis_CS%client
  ...
  sr_return_code = CS%client%put_tensor("EKE_shape"//CS%key_suffix, shape(G%geolonT), [2]) + sr_return_code
  ...
  if (smartredis_CS%colocated) then
    if (modulo(PE_here(),smartredis_CS%colocated_stride) == 0) then
      sr_return_code = CS%client%set_model_from_file(CS%model_key, trim(inputdir)//trim(model_filename), &
...
subroutine infer_meke(G, GV, US, CS, Time, MEKE, Rd_dx_h, u, v, tv, h, dt)
  ...
  type(unit_scale_type),                     intent(in)    :: US         !< A dimensional unit scaling type
  type(time_type),                           intent(in)    :: Time       !< The current model time.
  type(smartredis_meke_CS_type),             intent(in)    :: CS !< Control structure for inferring MEKE using SmartRedis
  ...
  real(kind=c_float), dimension(SIZE(mke),num_features) :: features_array
  real(kind=c_float), dimension(SIZE(mke)) :: MEKE_vec
  ...
  features_array(:,rv_idx) = pack(rv_z,.true.)
  call cpu_clock_begin(CS%id_put_tensor)
  sr_return_code = CS%client%put_tensor("features"//CS%key_suffix, features_array, shape(features_array))
  ...
  sr_return_code = CS%client%unpack_tensor( model_out(1), MEKE_vec, shape(MEKE_vec) )
  ...
  if (CS%online_analysis) then
    ...
    sr_return_code = CS%client%put_tensor(trim("EKE_")//trim(adjustl(time_suffix))//CS%key_suffix, MEKE, shape(MEKE))
  endif
